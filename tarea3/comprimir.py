...
#############################################################
# Asignar a False si no se cuenta con la libreria			#
dibujar = False  # mostrar el grafo con networkx 	 		#
debug = False #habilitar los print							#
#############################################################
...
def leerArchivo(nombreArchivo):
	try:
		...
		return '''Huffman code is a lookup table thats generated by a compression scheme known as 
		the Huffman algorithm. While the table itself is only a part of the complete algorithm, its 
...
def guardarArchivo(cadena):
	archivo = open("entradaComprimida.dat", "w")
...
def comprimir(cadena, codigos):
	...
	for caracter in cadena:
		# buscamos el caracter en la lista de codigos
...
def descomprimir(cadenaBinaria, nodos):
	msnDescomprimido = ""  
	while len(cadenaBinaria) > 0:
		nodoRaiz = nodos[-1][0][1]
		# buscar el sig. nodo hasta que sea un letra(STR)
		repeticiones = 0
		while type(nodoRaiz) != type("cadena"):
			# buscamos el nodo inicial en  la lista de nodos 
			for tuplaNodos, valorArista in nodos:
				# y verificamos que cuadre el valor de la arista
				# con el elemento de la cadenaBinaria
				if (tuplaNodos[1] == nodoRaiz) and (valorArista == int(cadenaBinaria[0])):
					# eliminamos el elemento procesado de la cadenaBinaria
					cadenaBinaria = cadenaBinaria[1:]
					# reasignamos la nueva raiz a buscar
					nodoRaiz = tuplaNodos[0]
					repeticiones += 1
					break
		msnDescomprimido += nodoRaiz
...
def main(nombreArchivo):
	...
	# leemos el archivo
	cadena = leerArchivo(nombreArchivo)
	...
	guardarArchivo(msnComprimido)
	
	# rescatar la cadena original a partir de la binaria(descomprimir)
	msnDescomprimido = descomprimir(msnComprimido, nodos)
